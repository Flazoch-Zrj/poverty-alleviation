...
-- 志愿服务模块增强 — 数据库迁移脚本
-- 数据库: poverty_db
-- 说明: 直接在原有数据库上执行，不影响已有数据
-- 执行: mysql -u root -p poverty_db < volunteer-upgrade.sql
-- ============================================================

USE `poverty_db`;

-- ============================================================
-- 1. volunteer_record 表增强
-- ============================================================
...
-- ============================================================
-- 2. volunteer_activity 表增强
-- ============================================================
ALTER TABLE `volunteer_activity`
  ADD COLUMN `activity_type`    VARCHAR(30)  DEFAULT '综合' COMMENT '活动分类: 走访慰问/技能培训/物资分发/环境整治/医疗义诊/教育辅导' AFTER `description`,
  ...
  MODIFY COLUMN `status`        TINYINT      DEFAULT 1 COMMENT '状态: 0 草稿/1 招募中/2 进行中/3 已结束/4 已取消',
  ADD INDEX `idx_family` (`family_id`),
...
CREATE TABLE `volunteer_level` (
  ...
  `level_name` VARCHAR(50)  NOT NULL COMMENT '等级名称（如: 一星志愿者）',
...
CREATE TABLE `volunteer_certificate` (
  ...
  `source_id`   BIGINT       DEFAULT NULL COMMENT '来源（活动/培训 ID）',
  `cert_name`   VARCHAR(200) DEFAULT NULL COMMENT '证书名称',
  `cert_number` VARCHAR(50)  NOT NULL COMMENT '证书编号（如 VOL-2026-XXXXXXXX）',
...
-- 完成
-- ============================================================
SELECT '志愿服务模块数据库升级完成' AS result;
